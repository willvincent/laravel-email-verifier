...
final readonly class QuickemailverificationEmailVerifier implements ExternalEmailVerifier
{
    ...
    {
        /** @var string $apiKey */
        $apiKey = $this->config->get('email-verifier.external.quickemailverification.api_key', '');
        /** @var string $endpoint */
        $endpoint = $this->config->get('email-verifier.external.quickemailverification.endpoint', '');
        /** @var int $timeout */
        ...
        if ($apiKey === '' || $endpoint === '') {
            return new EmailVerificationResult(
                ...
                reasons: [],
                meta: ['provider' => 'quickemailverification', 'configured' => false],
            );
        }
        ...
        try {
            ...
            // QuickEmailVerification indicates whether the request succeeded with "success"
            if (! (bool) ($data['success'] ?? false)) {
                return new EmailVerificationResult(
                    ...
                    meta: [
                        'provider' => 'quickemailverification',
                        'configured' => true,
                        'quickemailverification_success' => false,
                        'raw' => $data,
                    ],
                );
            }
            ...
            // Map QuickEmailVerification results into our thin statuses
            return match ($result) {
                'valid' => new EmailVerificationResult(true, 100, $email, [], $meta),

                // "invalid" is a hard reject; include their reason for debugging/tuning
                'invalid' => new EmailVerificationResult(
                    accepted: false,
                    score: 0,
                    normalizedEmail: $email,
                    reasons: ['external_rejected:'.($reason !== '' ? $reason : 'invalid')],
                    meta: $meta,
                ),
